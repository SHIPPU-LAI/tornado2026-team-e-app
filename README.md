...
**このリポジトリで統合することは、チームの決定事項です**（2026-09-03）。
当初は「統合する場所が無いので先に枠だけ用意した」ものでしたが、
使うことが決まりました。
...
| `features/introduce/` | 紹介（カード作成・表示・いいね） | **実装済み** |
| `features/auth/` | ログイン・ユーザー登録 | **実装済み** |
| `frontend/` | 画面（見た目） | 枠のみ |
| `public/` | CSS・画像素材 | 枠のみ |

**`features/register/` は削除した**（2026-09-03）。登録は `features/auth/` に
含まれている。経緯は `docs/設計書.md` 8-3 を参照。
...
/dev/introduce   カード作成・編集の動作確認用（バックエンド）
/dev/login       ログイン・新規登録の動作確認用（バックエンド）
/search          検索結果画面を置く場所 → 空き
/user            マッチング画面を置く場所 → 空き
/login           ログイン画面を置く場所 → 空き
/artisan         職人向け画面を置く場所 → 空き
/                いまは統合状況の一覧。差し替え可
```
...
| ~~ログイン機能の担当~~ | **解決済み（2026-09-03）。**担当者不在のまま`features/auth/`に実装した |
| ~~登録機能とログイン機能の境界~~ | **解決済み。**`features/auth/`に統合した。`features/register/`は削除した |
