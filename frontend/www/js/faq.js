...
(() => {
    ...
    // 下部メニューの「その他設定」は各HTMLに直接書かれている静的な
    // 項目のため、ここでFAQへのリンクに書き換える
    // （まだ使っていない項目のため、新しく増やすのではなく置き換える）。
    function convertOtherSettingsToFaq() {
        const bottomListEl = document.querySelector(".drawer-nav-list-bottom");
        if (!bottomListEl) return;

        const lastItem = bottomListEl.lastElementChild;
        if (!lastItem) return;

        const linkEl = lastItem.querySelector("a.drawer-nav-item");
        if (!linkEl) return;

        linkEl.href = `${BASE}html/faq.html`;
        const iconEl = linkEl.querySelector("img, .drawer-nav-icon");
        if (iconEl && iconEl.tagName === "IMG") {
            iconEl.src = `${BASE}images/faq.png`;
        }
        const labelEl = linkEl.querySelector("span:last-child");
        if (labelEl) labelEl.textContent = "よくある質問";
    }

    document.addEventListener("DOMContentLoaded", () => {
        renderNav();
        convertOtherSettingsToFaq();
    });
})();